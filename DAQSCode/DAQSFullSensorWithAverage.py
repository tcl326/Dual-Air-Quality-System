...
##                print we,ae,gasName,temp
        ...
        if started and Time1!=None and(time()-Time1>=interval):
           ...
           message={"DeviceId":boardNo,"TIME":str(Time),"O3":str(o3),"SO2":str(so2),"NO2":str(no2),"CO":str(co),"PM10":str(pm10),"PM1":str(pm1),"PM2":str(pm2),"LAT":str(Lat),"LONG":str(Lon),
                    'TEMP':str(Temp),'HUMID':str(Humidity),'SPEED':str(Speed),'CLIMB':str(Climb),'HEAD':Head,'EPS':EPS, 'EPX':EPX,'EPV':EPV,'EPT':EPT,'ALT':Alt}
           print('sending...')
           try:
